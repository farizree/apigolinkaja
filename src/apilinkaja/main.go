...
import (
	...
	Clinkaja "apigolinkaja/src/apilinkaja/controllers/linkaja"
	// Clinkaja "apibaflinkaja/controllers/dev/linkaja/orderdata"
...
func main() {
	...
	addr, err := Conf.DetermineListenAddresslinkaja()
	if err != nil {
		logger.Fatal(err)
		logger.Println(err)
		logkoe.Info(err)
	...
	{
		//Modul linkaja
		v1.GET("linkaja/checksaldo/account", Clinkaja.GetCheckSaldo)
		v1.POST("linkaja/transfer", Clinkaja.PostTransfer)
		v1.GET("/status", Clinkaja.CheckReadiness)
	}
	...
	go func() {
		<-quit
		logger.Println("Server apilinkaja is shutting down...")
		...
		if err := server.Shutdown(ctx); err != nil {
			logger.Fatalf("Could not gracefully shutdown the server apilinkaja: %v\n", err)
		}
		close(done)
	}()

	logger.Println("Server apilinkaja is ready to handle requests at", subhost, addr)
	...
	<-done
	logger.Println("Server apilinkaja stopped")
